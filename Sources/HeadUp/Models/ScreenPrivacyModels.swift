...
enum ScreenPrivacyCalibrationStage: CaseIterable, Equatable {
    ...
    var title: String {
        switch self {
        case .idle: return ""
        case .center: return "正视工作区中心"
        case .left: return "看向最左侧工作位置"
        case .right: return "看向最右侧工作位置"
        case .up: return "看向最高工作位置"
        case .down: return "看向最低工作位置"
        }
    }
...
enum ScreenPrivacyRuntimeStatus: Equatable {
    ...
    var title: String {
        switch self {
        case .disabled: return "屏幕保护未开启"
        case .needsCalibration: return "需要校准工作区"
        case .calibrating(let stage): return stage.title
        case .watching: return "屏幕保护已就绪"
        case .coveringSoon: return "头部已离开工作区"
...
enum ScreenPrivacyBackgroundStyle: String, CaseIterable, Identifiable {
    ...
    var title: String { self == .blur ? "模糊并压暗" : "纯色遮挡" }
}
